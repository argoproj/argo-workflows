...
func (q *syncQueries) CreateSemaphoreLimit(ctx context.Context, name string, sizeLimit int) error {
	return q.sessionProxy.With(ctx, func(session db.Session) error {
		_, err := session.Collection(q.config.LimitTable).Insert(&LimitRecord{
			Name:      name,
			SizeLimit: sizeLimit,
		})
		return err
...
func (q *syncQueries) UpdateSemaphoreLimit(ctx context.Context, name string, sizeLimit int) error {
	return q.sessionProxy.With(ctx, func(session db.Session) error {
		resp, err := session.SQL().Update(q.config.LimitTable).
...
func (q *syncQueries) DeleteSemaphoreLimit(ctx context.Context, name string) error {
	return q.sessionProxy.With(ctx, func(session db.Session) error {
		_, err := session.SQL().DeleteFrom(q.config.LimitTable).
			Where(db.Cond{LimitNameField: name}).
			Exec()
		return err
	})
...
// State operations
func (q *syncQueries) GetCurrentState(ctx context.Context, sessionProxy *sqldb.SessionProxy, semaphoreName string, held bool) ([]StateRecord, error) {
	var states []StateRecord
	err := sessionProxy.With(ctx, func(session db.Session) error {
		states = []StateRecord{}
		return session.SQL().
...
func (q *syncQueries) GetCurrentHolders(ctx context.Context, sessionProxy *sqldb.SessionProxy, semaphoreName string) ([]StateRecord, error) {
	return q.GetCurrentState(ctx, sessionProxy, semaphoreName, true)
}
...
func (q *syncQueries) UpdateStateToHeld(ctx context.Context, sessionProxy *sqldb.SessionProxy, semaphoreName, holderKey, controllerName string) error {
	return sessionProxy.With(ctx, func(session db.Session) error {
...
func (q *syncQueries) InsertHeldState(ctx context.Context, sessionProxy *sqldb.SessionProxy, record *StateRecord) error {
	return sessionProxy.With(ctx, func(session db.Session) error {
		_, err := session.Collection(q.config.StateTable).Insert(record)
		return err
	})
...
func (q *syncQueries) GetPendingInQueue(ctx context.Context, sessionProxy *sqldb.SessionProxy, semaphoreName, holderKey, controllerName string) ([]StateRecord, error) {
...
func (q *syncQueries) InsertLock(ctx context.Context, record *LockRecord) error {
	return q.sessionProxy.With(ctx, func(session db.Session) error {
		_, err := session.Collection(q.config.LockTable).Insert(record)
		return err
	})
}
...
func (q *syncQueries) ExpireInactiveLocks(ctx context.Context, inactiveTimeout time.Duration) (int64, error) {
	...
	err := q.sessionProxy.With(ctx, func(session db.Session) error {
		...
		result, err := session.SQL().DeleteFrom(q.config.LockTable).
...
func (q *syncQueries) InsertControllerHealth(ctx context.Context, record *ControllerHealthRecord) error {
	return q.sessionProxy.With(ctx, func(session db.Session) error {
		_, err := session.Collection(q.config.ControllerTable).Insert(record)
		return err
	})
}
...
func (q *syncQueries) UpdateControllerTimestamp(ctx context.Context, controllerName string, timestamp time.Time) error {
	return q.sessionProxy.With(ctx, func(session db.Session) error {
		_, err := session.SQL().Update(q.config.ControllerTable).
