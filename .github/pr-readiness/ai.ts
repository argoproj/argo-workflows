...
export function parseAiVerdict(text: string | null | undefined): AiVerdict | null {
  ...
  for (const issue of obj.issues as unknown[]) {
    if (typeof issue !== 'object' || issue === null || Array.isArray(issue)) {
      return null;
    }
    const rec = issue as Record<string, unknown>;
    const issueKeys = Object.keys(rec).sort();
    if (issueKeys.length !== 2 || issueKeys[0] !== 'problem' || issueKeys[1] !== 'section') {
      return null;
    }
    if (typeof rec.section !== 'string' || !SECTIONS.has(rec.section) || typeof rec.problem !== 'string' || rec.problem.length > MAX_PROBLEM_LENGTH) {
      return null;
    }
  }
  return obj as unknown as AiVerdict;
}
