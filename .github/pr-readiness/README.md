...
- Draft conversion needs a **GitHub App token**: the default Actions token cannot toggle draft state (`Resource not accessible by integration` — verified live). It runs as the `argo-workflows-pr-readiness` app, whose credentials are the `PR_READINESS_APP_ID` (the app's **client ID**, which is what `actions/create-github-app-token` takes as `client-id`) and `PR_READINESS_APP_PRIVATE_KEY` secrets — the same pattern as `cherry-pick-single.yml`. Without the secrets the bot comments but does not draft.
  - The app needs **Pull requests: Read & write** *and* **Contents: Read & write**. The second is counter-intuitive for a draft toggle and nothing here uses it, but GitHub refuses the `convertPullRequestToDraft` mutation without it, on the stated grounds that "a git branch and commits will be generated as a result" ([discussion](https://github.com/cli/cli/discussions/6924)). With `pull_requests` alone the mutation fails with `Resource not accessible by integration` — the same error the default token gives, so the symptom looks identical to having no app at all.
  - Changing an installed app's permissions raises a request that stays **pending until accepted** on the installation. Until then the token still carries the old scopes and drafting keeps failing.
...
- `permissions: {}` at the top; the job grants the `GITHUB_TOKEN` only `pull-requests: write`, `contents: read`, `actions: read`. The draft app's credentials are the only other secrets.
- The draft app's token is **more privileged than the job's `GITHUB_TOKEN`**: GitHub forces `Contents: Read & write` on it (see above), so it can push code, even though this bot only ever sends it one GraphQL mutation. It is scoped to this repository, minted per run, and revoked when the job ends. Do not hand it to any other step, and do not widen the app's permissions further. The "never check out or execute PR-head code" rule above is what keeps that credential out of a contributor's reach.
