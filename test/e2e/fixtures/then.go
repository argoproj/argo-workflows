...
func (t *Then) ExpectWorkflow(block func(t *testing.T, metadata *metav1.ObjectMeta, status *wfv1.WorkflowStatus)) *Then {
	return t.ExpectWorkflowFromName(t.workflowName, block)
}

func (t *Then) ExpectWorkflowFromName(workflowName string, block func(t *testing.T, metadata *metav1.ObjectMeta, status *wfv1.WorkflowStatus)) *Then {
	if workflowName == "" {
