...
func ExecuteNode(req plugin.ExecuteNodeReq, resp *wfv1.NodeStatus) error {
	...
	h := &httpReq{}
	if err := req.Template.Plugin.UnmarshalTo("http", h); err != nil {
		if err == wfv1.ErrPluginTemplateNotRequiredType {
			return nil
		}
		return err
	}
	...
	work.Add(task{req.Workflow.Namespace + "/" + req.Workflow.Name + "/" + req.Node.Name, r})
