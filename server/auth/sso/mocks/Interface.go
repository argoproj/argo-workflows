...
import (
	...
	sso "github.com/argoproj/argo/server/auth/sso"
)
...
func (_m *Interface) Authorize(ctx context.Context, authorization string) (*sso.Claims, error) {
	ret := _m.Called(ctx, authorization)

	var r0 *sso.Claims
	if rf, ok := ret.Get(0).(func(context.Context, string) *sso.Claims); ok {
		r0 = rf(ctx, authorization)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*sso.Claims)
		}
	}
