...
// TestPKCEDisabledByDefault verifies that PKCE is off unless explicitly opted
// in via EnablePKCEAuthentication. Enabling PKCE is recommended by the OAuth
// 2.0 Security BCP and required by certain authorization servers.
func TestPKCEDisabledByDefault(t *testing.T) {
	t.Run("default: disabled", func(t *testing.T) {
		s := newSsoForTest(t, nil)
		assert.False(t, s.pkceEnabled)
	})
	t.Run("EnablePKCEAuthentication=true enables", func(t *testing.T) {
		s := newSsoForTest(t, func(c *Config) { c.EnablePKCEAuthentication = true })
...
// TestHandleRedirectOmitsPKCEWhenDisabled verifies the default (PKCE off)
// preserves the pre-PKCE wire format: no code_challenge param, plain
// redirect-URL cookie value.
