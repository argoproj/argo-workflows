...
func (as *argoServer) Run(ctx context.Context, port int, browserOpenFunc func(string)) {
	...
	workflowServer := workflow.NewServer(ctx, instanceIDService, offloadRepo, wfArchive, as.clients.Dynamic, wfStore, wfStore, wftmplStore, cwftmplInformer, config.WorkflowDefaults, &resourceCacheNamespace)
