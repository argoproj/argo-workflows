...
func (as *argoServer) newHTTPServer(ctx context.Context, port int, artifactServer *artifacts.ArtifactServer) *http.Server {
	...
	mux.HandleFunc("/workflow-artifacts/v2/artifact-descriptions/", artifactServer.GetArtifactDescription)
	mux.HandleFunc("/workflow-artifacts/v2/artifacts/", artifactServer.GetArtifact)
	mux.HandleFunc("/artifacts/", artifactServer.Redirect)
	mux.HandleFunc("/input-artifacts/", artifactServer.Redirect)
	mux.HandleFunc("/artifacts-by-uid/", artifactServer.Redirect)
	mux.HandleFunc("/input-artifacts-by-uid/", artifactServer.Redirect)
