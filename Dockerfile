...
RUN . hack/image_arch.sh && make dist/emissary-${IMAGE_OS}-${IMAGE_ARCH}
...
COPY --from=argo-build /go/src/github.com/argoproj/argo/dist/emissary-${IMAGE_OS}-* /usr/local/bin/emissary
ENTRYPOINT [ "argoexec" ]
