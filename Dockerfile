...
RUN --mount=id=plugins,type=cache,target=/root/.cache/go-build make httptemplate.so jsonrpc.so

RUN mkdir -p /plugins/available

RUN mv *.so /plugins/available/

RUN mkdir -p /plugins/enabled

RUN ln -s /plugins/available/httptemplate.so /plugins/enabled/
RUN ln -s /plugins/available/jsonrpc.so /plugins/enabled/

####################################################################################################

FROM builder as controller-build


WORKDIR /go/src/github.com/argoproj/argo
COPY . .

RUN --mount=id=workflow-controller,type=cache,target=/root/.cache/go-build make dist/workflow-controller
...
RUN --mount=id=argoexec,type=cache,target=/root/.cache/go-build make dist/argoexec
...
RUN --mount=id=argocli,type=cache,target=/root/.cache/go-build make dist/argo
...
FROM gcr.io/distroless/base as workflow-controller

USER 8737

COPY --chown=8737 --from=plugins-build /plugins /plugins
COPY --chown=8737 --from=controller-build /go/src/github.com/argoproj/argo/dist/workflow-controller /bin/

ENTRYPOINT [ "workflow-controller" ]

####################################################################################################

FROM gcr.io/distroless/base as argocli
