...
func NewRootCommand() *cobra.Command {
	var (
		...
		SemaphoreSizeCacheTTL   int    // --semaphore-size-cache-ttl
		burst                   int
		qps                     float32
	...
	command := cobra.Command{
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			if leaderElectionOff == "true" {
				...
				go wfController.Run(ctx, workflowWorkers, workflowTTLWorkers, podCleanupWorkers, cronWorkflowWorkers, workflowArchiveWorkers, time.Second*time.Duration(SemaphoreSizeCacheTTL))
	...
	command.Flags().IntVar(&SemaphoreSizeCacheTTL, "semaphore-size-cache-ttl", 0, "Duration in seconds semaphore size will be cached for before refetching from configmap")
