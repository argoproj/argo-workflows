...
func NewRootCommand() *cobra.Command {
	var (
		...
		semaphoreCacheTTL       int    // --semaphore-cache-ttl
		burst                   int
		qps                     float32
	...
	command := cobra.Command{
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			if leaderElectionOff == "true" {
				...
				go wfController.Run(ctx, workflowWorkers, workflowTTLWorkers, podCleanupWorkers, cronWorkflowWorkers, workflowArchiveWorkers, time.Second*time.Duration(semaphoreCacheTTL))
	...
	command.Flags().IntVar(&semaphoreCacheTTL, "semaphore-cache-ttl", 0, "Duration in seconds semaphore size will be cached for before refetching from configmap")
