...
func NewRootCommand() *cobra.Command {
	var (
		...
		archivedWorkflowWorkers int    // --archived-workflow-workers
		burst                   int
		qps                     float32
	...
	command := cobra.Command{
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			if leaderElectionOff == "true" {
				...
				go wfController.Run(ctx, workflowWorkers, workflowTTLWorkers, podCleanupWorkers, cronWorkflowWorkers, archivedWorkflowWorkers)
	...
	command.Flags().IntVar(&archivedWorkflowWorkers, "archived-workflow-workers", 16, "Number of archived workflow workers")
