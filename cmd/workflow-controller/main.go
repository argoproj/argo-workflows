...
func NewRootCommand() *cobra.Command {
	var (
		...
		workflowThrottleWorkers int    // --workflow-throttle-workers
		burst                   int
		qps                     float32
	...
	command := cobra.Command{
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			if leaderElectionOff == "true" {
				...
				go wfController.Run(ctx, workflowWorkers, workflowTTLWorkers, podCleanupWorkers, cronWorkflowWorkers, workflowArchiveWorkers, workflowThrottleWorkers)
	...
	command.Flags().IntVar(&workflowThrottleWorkers, "workflow-throttle-workers", 8, "Number of workflow throttle workers")
