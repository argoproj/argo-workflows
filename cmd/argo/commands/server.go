...
See %s`, help.ArgoServer),
		RunE: func(c *cobra.Command, args []string) error {
			...
			if secure {
				...
				log.Warn("You are running in insecure mode. Learn how to enable transport layer security: https://argo-workflows.readthedocs.io/en/release-3.5.2/tls/")
			}
			...
			if reflect.DeepEqual(modes, auth.Modes{auth.Server: true}) {
				log.Warn("You are running without client authentication. Learn how to enable client authentication: https://argo-workflows.readthedocs.io/en/release-3.5.2/argo-server-auth-mode/")
			}
