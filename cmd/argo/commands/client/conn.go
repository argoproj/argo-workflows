...
func AddAPIClientFlagsToCmd(cmd *cobra.Command) {
	...
	cmd.PersistentFlags().StringVar(&ArgoServerOpts.BaseHRef, "argo-base-href", os.Getenv("ARGO_BASE_HREF"), "Path to use with HTTP client due to Base HREF. Defaults to the ARGO_BASE_HREF environment variable.")
	cmd.PersistentFlags().StringVar(&ArgoServerOpts.RootPath, "argo-root-path", os.Getenv("ARGO_ROOT_PATH"), "API path prefix when Argo Server is behind ingress/proxy. Defaults to the ARGO_ROOT_PATH environment variable.")
