...
func AddAPIClientFlagsToCmd(cmd *cobra.Command) {
	...
	cmd.PersistentFlags().StringVar(&argoServerOpts.Path, "argo-path", os.Getenv("ARGO_BASE_HREF"), "An path to use with HTTP client (e.g. due to BASE_HREF). Defaults to the ARGO_BASE_HREF environment variable.")
	cmd.PersistentFlags().BoolVar(&argoServerOpts.HTTP, "argo-http", os.Getenv("ARGO_HTTP") == "true", "If true, use the HTTP client. Defaults to the ARGO_HTTP environment variable.")
