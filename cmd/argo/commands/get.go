...
func printWorkflowHelper(wf *wfv1.Workflow, getArgs getFlags) {
	...
	if printTree {
		...
		if getArgs.output == "wide" {
			_, _ = fmt.Fprintf(w, "%s\tTEMPLATE\tPODNAME\tNODENAME\tDURATION\tARTIFACTS\tMESSAGE\tRESOURCESDURATION\n", ansiFormat("STEP", FgDefault))
		} else {
			_, _ = fmt.Fprintf(w, "%s\tTEMPLATE\tPODNAME\tNODENAME\tDURATION\tMESSAGE\n", ansiFormat("STEP", FgDefault))
		}
