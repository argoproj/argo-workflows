...
function write-master-env {
  ...
  if [[ "${REGISTER_MASTER_KUBELET:-}" == "true" ]]; then
    # AX. Name resolution might not work early. Use IP instead.
...
AX_ENABLE_MASTER_KUBE_PROXY: $(yaml-quote ${AX_ENABLE_MASTER_KUBE_PROXY:-true})
AX_ENABLE_MASTER_FLUENTD: $(yaml-quote ${AX_ENABLE_MASTER_FLUENTD:-true})
AX_TCP_KEEPALIVE: $(yaml-quote ${AX_TCP_KEEPALIVE:-true})
DOCKER_LOG_OPTS: $(yaml-quote ${DOCKER_LOG_OPTS:-})
ENABLE_DOCKER_DEBUG: $(yaml-quote ${ENABLE_DOCKER_DEBUG:-true})
EOF
...
EOF
    ...
    if [ -n "${ETCD_SNAPSHOT_COUNT:-}" ]; then
      cat >>$file <<EOF
ETCD_SNAPSHOT_COUNT: $(yaml-quote ${ETCD_SNAPSHOT_COUNT})
EOF
    fi
