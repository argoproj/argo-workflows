...
num_nodes: $(echo "${AX_NUM_NODES_MAX:-}" | sed -e "s/'/''/g")
e2e_storage_test_environment: '$(echo "$E2E_STORAGE_TEST_ENVIRONMENT" | sed -e "s/'/''/g")'
kube_uid: '$(echo "${KUBE_UID}" | sed -e "s/'/''/g")'
initial_etcd_cluster: '$(echo "${INITIAL_ETCD_CLUSTER:-}" | sed -e "s/'/''/g")'
etcd_snapshot_count: '$(echo "${ETCD_SNAPSHOT_COUNT:-}" | sed -e "s/'/''/g")'
hostname: $(hostname -s)
ax_compute_unit_max: '$(echo "${AX_COMPUTE_UNIT_MAX:-}" | sed -e "s/'/''/g")'
apiserver_cpu_req: '$(echo "${API_SERVER_CPU_REQ:-}" | sed -e "s/'/''/g")'
apiserver_mem_req: '$(echo "${API_SERVER_MEM_REQ:-}" | sed -e "s/'/''/g")'
apiserver_throttling: '$(echo "${API_SERVER_THROTTLING:-}" | sed -e "s/'/''/g")'
kubecontroller_cpu_req: '$(echo "${KUBE_CONTROLLER_CPU_REQ:-}" | sed -e "s/'/''/g")'
kubecontroller_ds_cache: '$(echo "${DAEMONSET_CACHE:-}" | sed -e "s/'/''/g")'
kubecontroller_rs_cache: '$(echo "${REPLICASET_CACHE:-}" | sed -e "s/'/''/g")'
kubecontroller_rc_cache: '$(echo "${RC_CACHE:-}" | sed -e "s/'/''/g")'
kubecontroller_dp_batch: '$(echo "${DEPLOYMENT_SYNC_BATCH:-}" | sed -e "s/'/''/g")'
kubecontroller_ep_batch: '$(echo "${ENDPOINT_SYNC_BATCH:-}" | sed -e "s/'/''/g")'
kubecontroller_gc_batch: '$(echo "${GARBAGE_COLLECTOR_SYNC_BATCH:-}" | sed -e "s/'/''/g")'
kubecontroller_ns_batch: '$(echo "${NAMESPACE_SYNC_BATCH:-}" | sed -e "s/'/''/g")'
kubecontroller_rs_batch: '$(echo "${REPLICASET_SYNC_BATCH:-}" | sed -e "s/'/''/g")'
kubecontroller_rq_batch: '$(echo "${RESOURCE_QUOTA_SYNC_BATCH:-}" | sed -e "s/'/''/g")'
kubecontroller_svc_batch: '$(echo "${SERVICE_SYNC_BATCH:-}" | sed -e "s/'/''/g")'
kubecontroller_sat_batch: '$(echo "${SA_TOKEN_SYNC_BATCH:-}" | sed -e "s/'/''/g")'
kubecontroller_rc_batch: '$(echo "${RC_SYNC_BATCH:-}" | sed -e "s/'/''/g")'
kubesched_cpu_req: '$(echo "${KUBE_SCHED_CPU_REQ:-}" | sed -e "s/'/''/g")'
kubesched_mem_req: '$(echo "${KUBE_SCHED_MEM_REQ:-}" | sed -e "s/'/''/g")'
kubesched_api_qps: '$(echo "${KUBE_SCHED_API_QPS:-}" | sed -e "s/'/''/g")'
kubesched_api_burst: '$(echo "${KUBE_SCHED_API_BURST:-}" | sed -e "s/'/''/g")'
resched_cpu_req: '$(echo "${KUBE_RESCHED_CPU_REQ:-}" | sed -e "s/'/''/g")'
resched_mem_req: '$(echo "${KUBE_RESCHED_MEM_REQ:-}" | sed -e "s/'/''/g")'
ax_enable_master_kube_proxy: '$(echo "${AX_ENABLE_MASTER_KUBE_PROXY:-}" | sed -e "s/'/''/g")'
ax_enable_master_fluentd: '$(echo "${AX_ENABLE_MASTER_FLUENTD:-}" | sed -e "s/'/''/g")'
ax_tcp_keepalive: '$(echo "${AX_TCP_KEEPALIVE:-}" | sed -e "s/'/''/g")'
EOF
...
EOF
    ...
    if [ "${AX_CONFIG_MASTER_KUBELET:-false}" == "true" ]; then
      cat <<EOF >>/srv/salt-overlay/pillar/cluster-params.sls
kubelet_api_qps: '$(echo "${MASTER_KUBELET_API_QPS}" | sed -e "s/'/''/g")'
kubelet_event_qps: '$(echo "${MASTER_KUBELET_EVENT_QPS}" | sed -e "s/'/''/g")'
kubelet_registry_qps: '$(echo "${MASTER_KUBELET_REGISTRY_QPS}" | sed -e "s/'/''/g")'
kubelet_api_burst: '$(echo "${MASTER_KUBELET_API_BURST}" | sed -e "s/'/''/g")'
kubelet_event_burst: '$(echo "${MASTER_KUBELET_EVENT_BURST}" | sed -e "s/'/''/g")'
kubelet_registry_burst: '$(echo "${MASTER_KUBELET_REGISTRY_BURST}" | sed -e "s/'/''/g")'
kubelet_max_pod: '$(echo "${MASTER_MAX_POD}" | sed -e "s/'/''/g")'
kubelet_max_open_file: '$(echo "${MASTER_KUBELET_MAX_OPEN_FILE}" | sed -e "s/'/''/g")'
kubelet_kubesys_reserved: '$(echo "${MASTER_KUBE_SYS_RESERVED}" | sed -e "s/'/''/g")'
EOF
    fi
    if [ "${AX_CONFIG_MINION_KUBELET:-false}" == "true" ]; then
      cat <<EOF >>/srv/salt-overlay/pillar/cluster-params.sls
kubelet_api_qps: '$(echo "${MINION_KUBELET_API_QPS}" | sed -e "s/'/''/g")'
kubelet_event_qps: '$(echo "${MINION_KUBELET_EVENT_QPS}" | sed -e "s/'/''/g")'
kubelet_registry_qps: '$(echo "${MINION_KUBELET_REGISTRY_QPS}" | sed -e "s/'/''/g")'
kubelet_api_burst: '$(echo "${MINION_KUBELET_API_BURST}" | sed -e "s/'/''/g")'
kubelet_event_burst: '$(echo "${MINION_KUBELET_EVENT_BURST}" | sed -e "s/'/''/g")'
kubelet_registry_burst: '$(echo "${MINION_KUBELET_REGISTRY_BURST}" | sed -e "s/'/''/g")'
kubelet_max_pod: '$(echo "${MINION_MAX_POD}" | sed -e "s/'/''/g")'
kubelet_max_open_file: '$(echo "${MINION_KUBELET_MAX_OPEN_FILE}" | sed -e "s/'/''/g")'
kubelet_kubesys_reserved: '$(echo "${MINION_KUBE_SYS_RESERVED}" | sed -e "s/'/''/g")'
kubelet_min_container_gc_ttl: '$(echo "${MIN_CONTAINER_GC_TTL}" | sed -e "s/'/''/g")'
kubelet_max_dead_containers: '$(echo "${MAX_DEAD_CONTAINERS}" | sed -e "s/'/''/g")'
kubelet_max_dead_contaienrs_per_container: '$(echo "${MAX_DEAD_CONTAINERS_PER_CONTAINER}" | sed -e "s/'/''/g")'
EOF
    fi
}
...
function configure-salt() {
  ...
  # AX: enable same docker log options on master and minion
  if [[ -n "${DOCKER_LOG_OPTS}" ]]; then
    DOCKER_OPTS="${DOCKER_OPTS:-} ${DOCKER_LOG_OPTS}"
  fi

  if [[ "${ENABLE_DOCKER_DEBUG}" == "true" ]]; then
...
if [[ -z "${is_push}" ]]; then
  ...
  if [[ "${KUBERNETES_MASTER}" == "true" ]]; then
    ...
    # Need to create token for master kube-proxy.
