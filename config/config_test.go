...
func TestSanitize(t *testing.T) {
	tests := []struct {
		...
		{Config{Links: []*wfv1.Link{{URL: "http://${metadata.name}.logging-facility?podName=${metadata.name}"}}}, ""},
		{Config{Links: []*wfv1.Link{{URL: "javascript:${metadata.name}"}}}, "protocol javascript is not allowed"},
	}
