...
func TestLabel(t *testing.T) {
	...
	t.Run("NotEmpty", func(t *testing.T) {
		...
		if require.NotEmpty(t, wf.Labels) {
			require.Equal(t, strings.Repeat("x", 62)+"y", wf.Labels[common.LabelKeyCreator], "creator is truncated")
			require.Equal(t, "my.at.email", wf.Labels[common.LabelKeyCreatorEmail], "'@' is replaced by '.at.'")
			require.Equal(t, "username", wf.Labels[common.LabelKeyCreatorPreferredUsername], "username is matching")
		}
	})
	t.Run("TooLongHyphen", func(t *testing.T) {
		...
		if require.NotEmpty(t, wf.Labels) {
			require.Equal(t, "y", wf.Labels[common.LabelKeyCreator])
		}
	})
	t.Run("InvalidDNSNames", func(t *testing.T) {
		...
		if require.NotEmpty(t, wf.Labels) {
			require.Equal(t, strings.Repeat("y", 35), wf.Labels[common.LabelKeyCreator])
			require.Equal(t, "us-er-name", wf.Labels[common.LabelKeyCreatorPreferredUsername], "username is truncated")
		}
	})
	t.Run("InvalidDNSNamesWithMidDashes", func(t *testing.T) {
		...
		if require.NotEmpty(t, wf.Labels) {
			require.Equal(t, strings.Repeat("x", 20), wf.Labels[common.LabelKeyCreator])
		}
	})
		...
		for _, testCase := range []struct {
			...
			t.Run(testCase.name, func(t *testing.T) {
				...
				for k, expectedValue := range testCase.output.creatorLabelsToHave {
					require.Equal(t, expectedValue, labels[k])
				}
				for _, creatorLabelKey := range testCase.output.creatorLabelsNotToHave {
					_, ok := labels[creatorLabelKey]
					require.Falsef(t, ok, "should not have the creator label, \"%s\"", creatorLabelKey)
				}
			})
