...
func TestLabel(t *testing.T) {
	...
	t.Run("NotEmpty", func(t *testing.T) {
		wf := &wfv1.Workflow{}
		Label(context.WithValue(context.TODO(), auth.ClaimsKey, types.Claims{"sub": strings.Repeat("x", 63) + "y", "email": "my@email"}), wf)
	...
	t.Run("TooLongHyphen", func(t *testing.T) {
		wf := &wfv1.Workflow{}
		Label(context.WithValue(context.TODO(), auth.ClaimsKey, types.Claims{"sub": strings.Repeat("-", 63) + "y"}), wf)
		if assert.NotEmpty(t, wf.Labels) {
	...
	t.Run("InvalidDNSNames", func(t *testing.T) {
		wf := &wfv1.Workflow{}
		Label(context.WithValue(context.TODO(), auth.ClaimsKey, types.Claims{"sub": "!@#$%^&*()--__" + strings.Repeat("y", 35) + "__--!@#$%^&*()"}), wf)
		if assert.NotEmpty(t, wf.Labels) {
	...
	t.Run("InvalidDNSNamesWithMidDashes", func(t *testing.T) {
		...
		Label(context.WithValue(context.TODO(), auth.ClaimsKey, types.Claims{"sub": sub}), wf)
		if assert.NotEmpty(t, wf.Labels) {
