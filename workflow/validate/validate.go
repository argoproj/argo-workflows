...
func newTemplateValidationCtx(wf *wfv1.Workflow, opts ValidateOpts) *templateValidationCtx {
	...
	globalParams[common.GlobalVarWorkflowName] = placeholderGenerator.GetPlaceholder()
	globalParams[common.GlobalVarWorkflowNamespace] = placeholderGenerator.GetPlaceholder()
	globalParams[common.GlobalVarWorkflowUID] = placeholderGenerator.GetPlaceholder()
...
func (args *FakeArguments) GetParameterByName(name string) *wfv1.Parameter {
	s := placeholderGenerator.GetPlaceholder()
...
func ValidateWorkflow(wftmplGetter templateresolution.WorkflowTemplateNamespacedGetter, wf *wfv1.Workflow, opts ValidateOpts) error {
	...
	for _, param := range wf.Spec.Arguments.Parameters {
		if param.Name != "" {
			if param.Value != nil {
				...
				ctx.globalParams["workflow.parameters."+param.Name] = placeholderGenerator.GetPlaceholder()
			}
		}
	}

	for k := range wf.ObjectMeta.Annotations {
		ctx.globalParams["workflow.annotations."+k] = placeholderGenerator.GetPlaceholder()
	}
	for k := range wf.ObjectMeta.Labels {
		ctx.globalParams["workflow.labels."+k] = placeholderGenerator.GetPlaceholder()
	}
	...
	if wf.Spec.OnExit != "" {
		...
		ctx.globalParams[common.GlobalVarWorkflowStatus] = placeholderGenerator.GetPlaceholder()
...
func (ctx *templateValidationCtx) validateTemplate(tmpl *wfv1.Template, tmplCtx *templateresolution.Context, args wfv1.ArgumentsProvider, extraScope map[string]interface{}) error {
	...
	if tmpl.IsPodType() {
		localParams[common.LocalVarPodName] = placeholderGenerator.GetPlaceholder()
		scope[common.LocalVarPodName] = placeholderGenerator.GetPlaceholder()
	}
	if tmpl.IsLeaf() {
...
func (ctx *templateValidationCtx) addOutputsToScope(tmpl *wfv1.Template, prefix string, scope map[string]interface{}, aggregate bool, isAncestor bool) {
	...
	for _, param := range tmpl.Outputs.Parameters {
		...
		if param.GlobalName != "" && !isParameter(param.GlobalName) {
			...
			ctx.globalParams[globalParamName] = placeholderGenerator.GetPlaceholder()
		}
	}
	for _, art := range tmpl.Outputs.Artifacts {
		...
		if art.GlobalName != "" && !isParameter(art.GlobalName) {
			...
			ctx.globalParams[globalArtName] = placeholderGenerator.GetPlaceholder()
		}
	}
	if aggregate {
		switch tmpl.GetType() {
