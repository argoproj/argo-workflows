...
func (we *WorkflowExecutor) FinalizeOutput(ctx context.Context) {
	err := retryutil.OnError(wait.Backoff{
		...
		if apierr.IsForbidden(err) {
			log.WithError(err).Warn("failed to patch task set, falling back to legacy/insecure pod patch, see https://argo-workflows.readthedocs.io/en/release-3.5.2/workflow-rbac/")
