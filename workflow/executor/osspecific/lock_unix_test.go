...
func TestMain(m *testing.M) {
	if os.Getenv("GO_WANT_LOCK_HELPER_PROCESS") == "1" { //nolint:forbidigo // env is the only channel to a re-exec'd test helper process
		runLockHelper()
		return
	}
	os.Exit(m.Run())
...
func runLockHelper() {
	path := os.Getenv("GO_LOCK_HELPER_PATH") //nolint:forbidigo // env is the only channel to a re-exec'd test helper process
	f, err := Acquire(path)
	if err != nil {
...
func TestWaitForSharedLock_UnblocksOnProcessKill(t *testing.T) {
	...
	cmd.Env = append(os.Environ(), "GO_WANT_LOCK_HELPER_PROCESS=1", "GO_LOCK_HELPER_PATH="+path) //nolint:forbidigo // env is the only channel to a re-exec'd test helper process
	...
	case err := <-done:
		require.NoError(t, err)
	case <-time.After(2 * time.Second):
		t.Fatal("WaitForSharedLock did not return within 2s of subprocess SIGKILL")
	}
}
