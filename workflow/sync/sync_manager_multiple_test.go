...
func setupMultipleLockManagersWithExistence(t *testing.T, dbType sqldb.DBType, semaphoreSize int, workflowExists1, workflowExists2 WorkflowExists) (context.Context, func(), *Manager, *Manager) {
	...
	syncMgr1 := createLockManager(ctx, info.Session, &cfg, func(_ context.Context, _ string) (int, error) { return 2, nil }, func(key string) {}, workflowExists1)
	require.NotNil(t, syncMgr1)
	require.NotNil(t, syncMgr1.dbInfo.Session)
	// Second controller
	cfg.ControllerName = "test2"
	syncMgr2 := createLockManager(ctx, info.Session, &cfg, func(_ context.Context, _ string) (int, error) { return 2, nil }, func(key string) {}, workflowExists2)
