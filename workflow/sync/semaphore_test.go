...
// the sibling was silently dropped from the queue - it missed the direct notification from
// notifyWaiters on release and only resumed on the next full reconciliation - while the
// acquiring node's own entry was left behind as a stale front-of-queue entry.
