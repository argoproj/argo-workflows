...
func TestInactiveControllerDBSemaphore(t *testing.T) {
	for _, dbType := range testDBTypes {
		t.Run(string(dbType), func(t *testing.T) {
			...
			s, info, deferfunc := createTestDatabaseSemaphore(t, "bar", "foo", 1, 0, nextWorkflow, dbType)
			defer deferfunc()
			...
			now := time.Now()
			require.NoError(t, s.addToQueue("foo/wf-01", 0, now))
			require.NoError(t, s.addToQueue("foo/wf-02", 0, now.Add(time.Second)))

			// Try to acquire - this should fail because the controller is considered inactive
			tx := &transaction{db: &info.session}
			acquired, _, _ := s.tryAcquire("foo/wf-01", tx)
...
func TestOtherControllerDBSemaphore(t *testing.T) {
	for _, dbType := range testDBTypes {
		t.Run(string(dbType), func(t *testing.T) {
			...
			now := time.Now()
			require.NoError(t, s.addToQueue("foo/our-wf-01", 0, now.Add(time.Second)))

			// Try to acquire - this should fail because the other controller's item is first in line
			tx := &transaction{db: &info.session}
			acquired, _, _ := s.tryAcquire("foo/our-wf-01", tx)
...
func TestMutexAndSemaphoreWithSameName(t *testing.T) {
	for _, dbType := range testDBTypes {
		t.Run(string(dbType), func(t *testing.T) {
			...
			semaphore, info, deferfunc := createTestDatabaseSemaphore(t, "shared-name", "foo", 2, 0, nextWorkflow, dbType)
			defer deferfunc()
			...
			// Mutex workflow 1
			tx := &transaction{db: &info.session}
			require.NoError(t, mutex.addToQueue("foo/wf-mutex-1", 0, now))
			mutexAcquired1, _, _ := mutex.tryAcquire("foo/wf-mutex-1", tx)
			assert.True(t, mutexAcquired1, "Mutex should be acquired by first workflow")

			// Semaphore workflow 1
			require.NoError(t, semaphore.addToQueue("foo/wf-sem-1", 0, now))
			semAcquired1, _, _ := semaphore.tryAcquire("foo/wf-sem-1", tx)
			assert.True(t, semAcquired1, "Semaphore should be acquired by first workflow")

			// Verify the mutex can't be acquired again
			require.NoError(t, mutex.addToQueue("foo/wf-mutex-2", 0, now))
			mutexAcquired2, _, _ := mutex.tryAcquire("foo/wf-mutex-2", tx)
			...
			require.NoError(t, semaphore.addToQueue("foo/wf-sem-2", 0, now))
			semAcquired2, _, _ := semaphore.tryAcquire("foo/wf-sem-2", tx)
			assert.True(t, semAcquired2, "Semaphore should be acquired by second workflow")

			// But not a third time (because limit=2)
			require.NoError(t, semaphore.addToQueue("foo/wf-sem-3", 0, now))
			semAcquired3, _, _ := semaphore.tryAcquire("foo/wf-sem-3", tx)
			...
			mutexAcquired2Again, _, _ := mutex.tryAcquire("foo/wf-mutex-2", tx)
			...
			semAcquired3Again, _, _ := semaphore.tryAcquire("foo/wf-sem-3", tx)
			...
			require.NoError(t, semaphore.addToQueue("foo/wf-sem-4", 0, now))
			semAcquired4, _, _ := semaphore.tryAcquire("foo/wf-sem-4", tx)
			assert.False(t, semAcquired4, "Semaphore should not be acquired fourth time (at capacity again)")

			// The mutex should still be held
			mutexAcquired3, _, _ := mutex.tryAcquire("foo/wf-mutex-3", tx)
	...
	for _, dbType := range testDBTypes {
		t.Run(string(dbType), func(t *testing.T) {
			t.Run("RefreshesAfterTTL", func(t *testing.T) {
				...
				s, info, deferfunc := createTestDatabaseSemaphore(t, "test-semaphore", "foo", 5, cacheTTL, nextWorkflow, dbType)
				defer deferfunc()

				// First call to getLimit() should return the initial limit
				initialLimit := s.getLimit()
			...
			t.Run("ZeroTTLAlwaysRefreshes", func(t *testing.T) {
				...
				s, info, deferfunc := createTestDatabaseSemaphore(t, "test-semaphore-zero", "foo", 5, 0, nextWorkflow, dbType)
				defer deferfunc()
