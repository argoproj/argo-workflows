...
func TestMutexAndSemaphoreWithSameName(t *testing.T) {
	for _, dbType := range testDBTypes {
		t.Run(string(dbType), func(t *testing.T) {
			...
			mutexAcquired2Again, _ := mutex.tryAcquire("foo/wf-mutex-2", tx)
			...
			semAcquired3Again, _ := semaphore.tryAcquire("foo/wf-sem-3", tx)
			...
			mutexAcquired3, _ := mutex.tryAcquire("foo/wf-mutex-3", tx)
