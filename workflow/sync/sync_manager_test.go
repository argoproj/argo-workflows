...
func TestSemaphoreWfLevel(t *testing.T) {
	...
	t.Run("InitializeConfigMapInitFailurePoisonsNotFatal", func(t *testing.T) {
		...
		kubeClient := fake.NewSimpleClientset() // no ConfigMap created
		syncManager := NewLockManager(ctx, kubeClient, "", nil, GetSyncLimitFunc(kubeClient), func(key string) {
		}, WorkflowExistenceFunc)
	...
	t.Run("InitializeFailsClosedWhenDBUnavailable", func(t *testing.T) {
		...
		require.Nil(t, syncManager.dbInfo.session)
...
func TestDatabaseInitializeLoweredLimitAfterRestart(t *testing.T) {
	for _, dbType := range testDBTypes {
		t.Run(string(dbType), func(t *testing.T) {
			...
			// Cancel the context before cleanup closes the database session so
			// the managers' background notifier goroutines stop first (upstream
			// these tests rely on t.Context() being canceled at test end).
			ctx, cancel := context.WithCancel(context.Background())
			defer cancel()

			// Original limit: 2.
			_, err = info.session.SQL().Exec("INSERT INTO sync_limit (name, sizelimit) VALUES (?, ?)", dbLimitKey, 2)
			require.NoError(t, err)

			// --- Controller instance #1: two workflows acquire the semaphore. ---
			mgr1 := createLockManager(ctx, info.session, &syncConfig, nil, func(key string) {}, WorkflowExistenceFunc)
			...
			_, err = info.session.SQL().Exec("UPDATE sync_limit SET sizelimit = ? WHERE name = ?", 1, dbLimitKey)
			require.NoError(t, err)

			// --- Controller instance #2: restart. Initialize from the running workflows. ---
			mgr2 := createLockManager(ctx, info.session, &syncConfig, nil, func(key string) {}, WorkflowExistenceFunc)
...
func TestDatabaseInitializeStaleHoldFailsWorkflow(t *testing.T) {
	for _, dbType := range testDBTypes {
		t.Run(string(dbType), func(t *testing.T) {
			...
			_, err = info.session.SQL().Exec("DELETE FROM sync_state WHERE name = ? AND workflowkey = ?", "sem/"+dbLimitKey, "default/stale-holder")
			require.NoError(t, err)
