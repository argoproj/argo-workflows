...
func TestParallelismUpdate(t *testing.T) {
	...
	base := time.Unix(0, 0)
	throttler.Add("a/0", 0, base)
	throttler.Add("b/0", 0, base.Add(time.Second))
	throttler.Add("c/0", 0, base.Add(2*time.Second))
	throttler.Add("d/0", 0, base.Add(3*time.Second))
	throttler.Add("e/0", 0, base.Add(4*time.Second))
	throttler.Add("f/0", 0, base.Add(5*time.Second))
