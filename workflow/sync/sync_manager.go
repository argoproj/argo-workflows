...
func NewLockManager(ctx context.Context, kubectlConfig kubernetes.Interface, namespace string, database *config.DatabaseSyncConfig, getSyncLimit GetSyncLimit, nextWorkflow NextWorkflow, isWFDeleted IsWorkflowDeleted) *Manager {
	var dbSession db.Session
	var dbConfig dbConfig
	if database != nil {
		var err error
		dbSession, err = sqldb.CreateDBSession(ctx, kubectlConfig, namespace, database.DBConfig)
		if err != nil {
			// Carry on anyway, but database sync locks won't work
			dbSession = nil
		}
		dbConfig.limitTable = defaultTable(database.LimitTableName, defaultLimitTableName)
	...
	if dbSession != nil {
		sm.backgroundNotifier(ctx, database.PollSeconds)
		sm.dbControllerHeartbeat(ctx, database.HeartbeatSeconds)
	}
	return sm
}
