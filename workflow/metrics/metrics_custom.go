...
func (m *Metrics) matchExistingMetric(metricSpec *wfv1.Prometheus) (*instrument, error) {
	...
	if inst, ok := m.allInstruments[key]; ok {
		// if val.instrument.name != metricSpec.Name {
		// 	return nil, fmt.Errorf("Name of metric %s is already set to %s, it cannot be changed", metricSpec.Name, val.instrument.description)
		// }
...
// func (m *Metrics) UpsertRealTimeCustomMetric(ctx context.Context, metricSpec *wfv1.Prometheus, ownerKey string, valueFunc RealTimeValueFunc) {
// 	m.createCustomMetric(metricSpec)
// 	//	metric.rtValueFunc = &valueFunc
// }
