...
func TestProcessArgsAbsentOptional(t *testing.T) {
	...
	t.Run("input default applies", func(t *testing.T) {
		...
		newTmpl, err := ProcessArgs(&tmpl, &sentinelArgs, globalParams, localParams, false, "", configMapStore)
		require.NoError(t, err)
	...
	t.Run("no default nor valueFrom fails terminally without requeue", func(t *testing.T) {
		...
		_, err := ProcessArgs(&tmpl, &sentinelArgs, globalParams, localParams, false, "", configMapStore)
		require.Error(t, err)
