...
func TestConfigMapCacheLoadHit(t *testing.T) {
	...
	c := cache.NewConfigMapCache("default", controller.kubeclientset[wfv1.ThisCluster], "whalesay-cache")
...
func TestConfigMapCacheLoadMiss(t *testing.T) {
	cancel, controller := newController()
	defer cancel()
	_, err := controller.kubeclientset[wfv1.ThisCluster].CoreV1().ConfigMaps("default").Create(&sampleConfigMapEmptyCacheEntry)
	assert.NoError(t, err)
	c := cache.NewConfigMapCache("default", controller.kubeclientset[wfv1.ThisCluster], "whalesay-cache")
	entry, err := c.Load("hi-there-world")
	assert.NoError(t, err)
