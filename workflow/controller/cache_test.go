...
func TestConfigMapCacheLoadHit(t *testing.T) {
	...
	c := cache.NewConfigMapCache(defaultManagedNamespace, controller.kubeclientset, "whalesay-cache")

	cm, err := controller.kubeclientset.CoreV1().ConfigMaps(defaultManagedNamespace).Get(ctx, sampleConfigMapCacheEntry.Name, metav1.GetOptions{})
	assert.NoError(t, err)
...
func TestConfigMapCacheLoadMiss(t *testing.T) {
	...
	ctx := context.Background()
	_, err := controller.kubeclientset.CoreV1().ConfigMaps(defaultManagedNamespace).Create(ctx, &sampleConfigMapEmptyCacheEntry, metav1.CreateOptions{})
	assert.NoError(t, err)
	c := cache.NewConfigMapCache(defaultManagedNamespace, controller.kubeclientset, "whalesay-cache")
	entry, err := c.Load(ctx, "hi-there-world")
	assert.NoError(t, err)
