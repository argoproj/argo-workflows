...
func TestStepsSkippedRefDynamicTemplateName(t *testing.T) {
	ctx := context.Background()
	cancel, controller := newController(wfv1.MustUnmarshalWorkflow(stepsSkippedRefDynamicTemplateName), wfv1.MustUnmarshalWorkflowTemplate(skippedRefConsumeWorkflowTemplate))
	defer cancel()

	woc := newWorkflowOperationCtx(wfv1.MustUnmarshalWorkflow(stepsSkippedRefDynamicTemplateName), controller)
	woc.operate(ctx)
...
func TestStepsWhenFalseSkipsDropPass(t *testing.T) {
	ctx := context.Background()
	cancel, controller := newController(wfv1.MustUnmarshalWorkflow(stepsWhenFalseSkippedRefDrop), wfv1.MustUnmarshalWorkflowTemplate(skippedRefConsumeWorkflowTemplate))
	defer cancel()

	woc := newWorkflowOperationCtx(wfv1.MustUnmarshalWorkflow(stepsWhenFalseSkippedRefDrop), controller)
	woc.operate(ctx)
