...
func (woc *wfOperationCtx) applyExecutionControl(ctx context.Context, pod *apiv1.Pod, wfNodesLock *sync.RWMutex) {
	...
	if _, onExitPod := pod.Labels[common.LabelKeyOnExit]; woc.GetShutdownStrategy().Enabled() && !woc.GetShutdownStrategy().ShouldExecute(onExitPod) {
	...
	if woc.workflowDeadline != nil {
		...
		woc.queuePodForCleanupAfter(pod.Name, terminateContainers, -time.Since(*woc.workflowDeadline))
	}
}
...
func (woc *wfOperationCtx) killDaemonedChildren(nodeID string) {
	...
	for _, childNode := range woc.wf.Status.Nodes {
		...
		woc.queuePodForCleanup(childNode.ID, terminateContainers)
