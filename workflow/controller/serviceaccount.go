...
func (woc *wfOperationCtx) getServiceAccountTokenVolume(ctx context.Context, cluster, serviceAccountName string) (*apiv1.Volume, *apiv1.VolumeMount, error) {
	secretName, err := woc.getServiceAccountTokenName(ctx, cluster, serviceAccountName)
	if err != nil {
		return nil, nil, err
	}
