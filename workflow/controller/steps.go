...
func (woc *wfOperationCtx) executeSteps(ctx context.Context, nodeName string, tmplCtx *templateresolution.Context, templateScope string, tmpl *wfv1.Template, orgTmpl wfv1.TemplateReferenceHolder, opts *executeTemplateOpts) (*wfv1.NodeStatus, error) {
	...
	if err != nil {
		node = woc.initializeExecutableNode(nodeName, wfv1.NodeTypeSteps, templateScope, tmpl, orgTmpl, opts.boundaryID, wfv1.NodeRunning, opts.nodeFlag, true)
	}
	...
	for i, stepGroup := range tmpl.Steps {
			...
			if len(prevStepGroupNode.Children) == 0 {
				...
				for _, childID := range prevStepGroupNode.Children {
					...
					for _, outNodeID := range outboundNodeIDs {
						...
						if err != nil {
							woc.log.Fatalf("was not able to obtain node name for %s", outNodeID)
							panic(fmt.Sprintf("could not obtain the out noden name for %s", outNodeID))
						}
						woc.addChildNode(outNodeName, sgNodeName)
					}
				}
			}
		}
		...
		for _, step := range stepGroup.Steps {
			...
			if err != nil {
				...
				woc.addSkippedNodeOutputsToScope(stepsCtx.tmplCtx, stepsCtx.scope, prefix, childNode, &step, true)
			}
		}
	}
...
func (woc *wfOperationCtx) executeStepGroup(ctx context.Context, stepGroup []wfv1.WorkflowStep, sgNodeName string, stepsCtx *stepsContext) (*wfv1.NodeStatus, error) {
	...
	for _, step := range stepGroup {
		...
		if err != nil {
			woc.initializeNode(childNodeName, wfv1.NodeTypeSkipped, stepTemplateScope, &step, stepsCtx.boundaryID, wfv1.NodeError, &wfv1.NodeFlag{}, true, err.Error())
		...
		if !proceed {
			if _, err := woc.wf.GetNodeByName(childNodeName); err != nil {
				...
				woc.initializeNode(childNodeName, wfv1.NodeTypeSkipped, stepTemplateScope, &step, stepsCtx.boundaryID, wfv1.NodeSkipped, &wfv1.NodeFlag{}, true, skipReason)
				woc.addChildNode(sgNodeName, childNodeName)
			}
			continue
		}
		...
		if stepsCtx.boundaryID == "" {
			woc.log.Warnf("boundaryID was nil")
		}
...
func (woc *wfOperationCtx) resolveReferences(stepGroup []wfv1.WorkflowStep, scope *wfScope) ([]wfv1.WorkflowStep, error) {
	...
	resolveStepReferences := func(i int, step wfv1.WorkflowStep, newStepGroup []wfv1.WorkflowStep) error {
		...
		if step.When != "" {
			...
			resolvedWhenStr, err := template.ReplaceStrictAny(string(whenBytes), mergedParams, []string{"steps", "tasks"})
			if err != nil {
		...
		newStepStr, err := template.ReplaceStrictAny(string(stepBytes), mergedParams, []string{"steps", "tasks"})
		if err != nil {
			if template.IsMissingVariableErr(err) {
				woc.requeue()
...
func (woc *wfOperationCtx) expandStep(step wfv1.WorkflowStep, scope *wfScope) ([]wfv1.WorkflowStep, error) {
	...
	for i, item := range items {
		var newStep wfv1.WorkflowStep
		newStepName, err := processItem(t, step.Name, i, item, &newStep, step.When, scope.getParametersAny(woc.globalParams))
		if err != nil {
			return nil, err
		}
