...
func TestWithSequenceExpressionPreservesGlobalScope(t *testing.T) {
	...
	t.Run("CheckExpressionWithItemAndInputs", func(t *testing.T) {
		...
		// Debug: Print all node names to understand the structure
		t.Logf("Workflow phase: %s", woc.wf.Status.Phase)
		t.Logf("Total nodes: %d", len(woc.wf.Status.Nodes))
		for nodeName, node := range woc.wf.Status.Nodes {
			t.Logf("Node: %s, DisplayName: %s, Type: %s, Phase: %s",
				nodeName, node.DisplayName, node.Type, node.Phase)
		}
