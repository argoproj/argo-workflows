...
func TestSidecarWithVolume(t *testing.T) {
	...
	pods, err := controller.kubeclientset[wfv1.ThisCluster].CoreV1().Pods(wf.ObjectMeta.Namespace).List(metav1.ListOptions{})
	assert.NoError(t, err)
...
func TestRetriesVariable(t *testing.T) {
	...
	pods, err := controller.kubeclientset[wfv1.ThisCluster].CoreV1().Pods("").List(metav1.ListOptions{})
...
func TestSidecarResourceLimits(t *testing.T) {
	...
	woc := newWorkflowOperationCtx(wf, controller)
	woc.operate()
	pod, err := controller.kubeclientset[wfv1.ThisCluster].CoreV1().Pods("").Get("hello-world", metav1.GetOptions{})
	assert.NoError(t, err)
...
func TestWorkflowSpecParam(t *testing.T) {
	...
	woc.operate()
	pod, err := controller.kubeclientset[wfv1.ThisCluster].CoreV1().Pods("").Get(wf.Name, metav1.GetOptions{})
	assert.NoError(t, err)
	found := false
...
func TestParamSubstitutionWithArtifact(t *testing.T) {
	...
	pods, err := woc.controller.kubeclientset[wfv1.ThisCluster].CoreV1().Pods("").List(metav1.ListOptions{})
	assert.NoError(t, err)
...
func TestResolveIOPathPlaceholders(t *testing.T) {
	...
	woc.operate()
	assert.Equal(t, wfv1.NodeRunning, woc.wf.Status.Phase)
	pods, err := woc.controller.kubeclientset[wfv1.ThisCluster].CoreV1().Pods(wf.ObjectMeta.Namespace).List(metav1.ListOptions{})
	assert.NoError(t, err)
	assert.True(t, len(pods.Items) > 0, "pod was not created successfully")
...
func TestResourceTemplate(t *testing.T) {
	...
	pod, err := controller.kubeclientset[wfv1.ThisCluster].CoreV1().Pods("").Get("resource-template", metav1.GetOptions{})
	if !assert.NoError(t, err) {
		t.Fatal(err)
	}
...
func TestPDBCreation(t *testing.T) {
	...
	woc := newWorkflowOperationCtx(wf, controller)
	woc.operate()
	pdb, _ := controller.kubeclientset[wfv1.ThisCluster].PolicyV1beta1().PodDisruptionBudgets("").Get(woc.wf.Name, metav1.GetOptions{})
	assert.Equal(t, pdb.Name, wf.Name)
	woc.markWorkflowSuccess()
	_, err := controller.kubeclientset[wfv1.ThisCluster].PolicyV1beta1().PodDisruptionBudgets("").Get(woc.wf.Name, metav1.GetOptions{})
...
func TestPDBCreationRaceDelete(t *testing.T) {
	...
	woc.operate()
	makePodsPhase(woc, apiv1.PodSucceeded)
	err := controller.kubeclientset[wfv1.ThisCluster].PolicyV1beta1().PodDisruptionBudgets("").Delete(woc.wf.Name, nil)
	assert.NoError(t, err)
	woc = newWorkflowOperationCtx(woc.wf, controller)
	woc.operate()
	assert.Equal(t, wfv1.NodeSucceeded, woc.wf.Status.Phase)
...
func TestConfigMapCacheLoadOperate(t *testing.T) {
	...
	assert.NoError(t, err)
	_, err = controller.kubeclientset[wfv1.ThisCluster].CoreV1().ConfigMaps("default").Create(&sampleConfigMapCacheEntry)
	assert.NoError(t, err)
...
func TestConfigMapCacheLoadOperateMaxAge(t *testing.T) {
	...
	_, err = controller.kubeclientset[wfv1.ThisCluster].CoreV1().ConfigMaps("default").Create(&nonExpiredEntry)
	assert.NoError(t, err)
	...
	_, err = controller.kubeclientset[wfv1.ThisCluster].CoreV1().ConfigMaps("default").Create(&expiredEntry)
	assert.NoError(t, err)
...
func TestConfigMapCacheSaveOperate(t *testing.T) {
	...
	cm, err := controller.kubeclientset[wfv1.ThisCluster].CoreV1().ConfigMaps("default").Get("whalesay-cache", metav1.GetOptions{})
	assert.NoError(t, err)
	assert.NotNil(t, cm)
...
`)
	...
	if assert.NoError(t, err) {
		...
		list, err := controller.kubeclientset[wfv1.ThisCluster].CoreV1().Pods("").List(metav1.ListOptions{})
		if assert.NoError(t, err) {
...
func TestStorageQuota(t *testing.T) {
	...
	controller.kubeclientset[wfv1.ThisCluster].(*fake.Clientset).BatchV1().(*batchfake.FakeBatchV1).Fake.PrependReactor("create", "persistentvolumeclaims", func(action k8stesting.Action) (bool, runtime.Object, error) {
