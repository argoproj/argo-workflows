...
const withItemsOptionalArtWF = `apiVersion: argoproj.io/v1alpha1
kind: Workflow
metadata:
...
func TestWithItemsOptionalArtWF(t *testing.T) {
	wf := wfv1.MustUnmarshalWorkflow(withItemsOptionalArtWF)
	cancel, controller := newController(wf)
	defer cancel()
