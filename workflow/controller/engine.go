...
func (e *Engine) reconcileExpandedChildren(ctx context.Context, task dag.Task) {
	...
	for _, childID := range taskNode.Children {
		...
		processedTmpl, procErr := common.ProcessArgs(ctx, resolvedTmpl, &args, e.woc.globalParams(), localParams, false, true, e.woc.wf.Namespace, e.woc.controller.typedConfigMapInformer.GetIndexer())
		if procErr != nil {
			continue
		}
