...
func TestWFLevelAutomountServiceAccountToken(t *testing.T) {
	woc := newWoc()
	_, err := util.CreateServiceAccountWithToken(woc.controller.kubeclientset0(), "", "foo", "foo-token")
	assert.NoError(t, err)
...
func TestTmplLevelExecutorServiceAccountName(t *testing.T) {
	...
	_, err := util.CreateServiceAccountWithToken(woc.controller.kubeclientset0(), "", "foo", "foo-token")
	assert.NoError(t, err)
	_, err = util.CreateServiceAccountWithToken(woc.controller.kubeclientset0(), "", "tmpl", "tmpl-token")
	assert.NoError(t, err)
