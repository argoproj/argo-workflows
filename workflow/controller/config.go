...
func (wfc *WorkflowController) updateConfig(v interface{}) error {
	...
	wfc.session = nil
	wfc.artifactRepositories = artifactrepositories.New(wfc.kubernetesInterfaces[common.LocalCluster], wfc.namespace, &wfc.Config.ArtifactRepository)
	...
	if persistence != nil {
		...
		session, tableName, err := sqldb.CreateDBSession(wfc.kubernetesInterfaces[common.LocalCluster], wfc.namespace, persistence)
		if err != nil {
			return err
		}
