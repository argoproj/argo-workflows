	...
	for _, taskName := range targetTasks {
		...
		if taskNode != nil && taskNode.Fulfilled() {
			if taskNode.Completed() {
				...
				_, _, err := woc.runOnExitNode(ctx, dagCtx.GetTask(taskName).GetExitTemplate(woc.execWf.Spec.Arguments), taskName, taskNode.Name, dagCtx.boundaryID, dagCtx.tmplCtx, "tasks."+taskName, taskNode.Outputs)
				if err != nil {
					return node, err
				}
			}
...
func (woc *wfOperationCtx) executeDAGTask(ctx context.Context, dagCtx *dagContext, taskName string) {
	...
	if node != nil && node.Fulfilled() {
		...
		if node.Completed() {
			...
			hasOnExitNode, onExitNode, err := woc.runOnExitNode(ctx, task.GetExitTemplate(woc.execWf.Spec.Arguments), task.Name, node.Name, dagCtx.boundaryID, dagCtx.tmplCtx, "tasks."+taskName, node.Outputs)
