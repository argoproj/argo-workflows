...
func (woc *wfOperationCtx) executeDAGTask(ctx context.Context, dagCtx *dagContext, taskName string) {
	...
	if dagCtx.GetTaskDependsLogic(taskName) != "" {
		...
		if err != nil {
			woc.initializeNode(nodeName, nil, wfv1.NodeTypeSkipped, dagTemplateScope, nil, task, dagCtx.boundaryID, wfv1.NodeError, &wfv1.NodeFlag{}, err.Error())
		...
		if !execute {
			...
			woc.initializeNode(nodeName, nil, wfv1.NodeTypeSkipped, dagTemplateScope, nil, task, dagCtx.boundaryID, wfv1.NodeOmitted, &wfv1.NodeFlag{}, "omitted: depends condition not met")
			connectDependencies(nodeName)
			return
		}
	}
	...
	if task.ShouldExpand() {
		...
		if len(expandedTasks) == 0 {
			...
			woc.initializeNode(nodeName, nil, wfv1.NodeTypeSkipped, dagTemplateScope, nil, task, dagCtx.boundaryID, wfv1.NodeSkipped, &wfv1.NodeFlag{}, skipReason)
			connectDependencies(nodeName)
		} else if taskGroupNode == nil {
			connectDependencies(nodeName)
			taskGroupNode = woc.initializeNode(nodeName, nil, wfv1.NodeTypeTaskGroup, dagTemplateScope, nil, task, dagCtx.boundaryID, wfv1.NodeRunning, &wfv1.NodeFlag{}, "")
		}
	}
	...
	for _, t := range expandedTasks {
		...
		if node == nil {
			...
			if err != nil {
				woc.initializeNode(taskNodeName, nil, wfv1.NodeTypeSkipped, dagTemplateScope, nil, task, dagCtx.boundaryID, wfv1.NodeError, &wfv1.NodeFlag{}, err.Error())
				continue
			}
			if !proceed {
				skipReason := fmt.Sprintf("when '%s' evaluated false", t.When)
				woc.initializeNode(taskNodeName, nil, wfv1.NodeTypeSkipped, dagTemplateScope, nil, task, dagCtx.boundaryID, wfv1.NodeSkipped, &wfv1.NodeFlag{}, skipReason)
				continue
			}
		}
