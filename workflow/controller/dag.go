...
func (d *dagContext) getTaskFromNode(node *wfv1.NodeStatus) *wfv1.DAGTask {
	for _, task := range d.tasks {
		...
		// It could be that this node resulted from a task generated by an expansion; check if that's the case.
		expandedTasks, err := expandTask(task)
		if err != nil {
			panic("could not expand task " + task.Name + " while looking for task corresponding to node " + node.Name + ": " + err.Error())
		}
		for _, expandedTask := range expandedTasks {
			if node.Name == d.taskNodeName(expandedTask.Name) {
				// If this node did indeed result from a task expansion, return the parent task, not the expanded task.
				return &task
			}
		}
	}
