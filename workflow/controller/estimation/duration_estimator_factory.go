...
type DurationEstimatorFactory interface {
	NewDurationEstimator(wf *wfv1.Workflow) (*DurationEstimator, error)
}

type durationEstimatorFactory struct {
	wfInformer cache.SharedIndexInformer
	hydrator   hydrator.Interface
	wfArchive  sqldb.WorkflowArchive
}

var _ DurationEstimatorFactory = &durationEstimatorFactory{}

func NewDurationEstimatorFactory(wfInformer cache.SharedIndexInformer, hydrator hydrator.Interface, wfArchive sqldb.WorkflowArchive) DurationEstimatorFactory {
	return &durationEstimatorFactory{wfInformer, hydrator, wfArchive}
}

func (woc *durationEstimatorFactory) NewDurationEstimator(wf *wfv1.Workflow) (*DurationEstimator, error) {
	for labelName, indexName := range map[string]string{
		...
		if exists {
			...
			if newestWf != nil {
				...
				return &DurationEstimator{wf, newestWf}, nil
			}
			...
			if len(workflows) > 0 {
				return &DurationEstimator{wf, &workflows[0]}, nil
			}
		}
	}
