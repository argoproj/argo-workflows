...
func (woc *wfOperationCtx) createWorkflowPod(nodeName string, mainCtr apiv1.Container, tmpl *wfv1.Template, opts *createWorkflowPodOpts) (*apiv1.Pod, error) {
	...
	location := woc.artifactRepository.ToArtifactLocation()
	for i, artifact := range tmpl.Inputs.Artifacts {
		if !artifact.HasLocation() {
			(&artifact).Merge(location)
			tmpl.Inputs.Artifacts[i] = artifact
		}
	}
	for i, artifact := range tmpl.Outputs.Artifacts {
		if !artifact.HasLocation() {
			(&artifact).Merge(location)
			tmpl.Outputs.Artifacts[i] = artifact
		}
	}
