...
func (woc *wfOperationCtx) addSkippedNodeOutputsToScope(tmplCtx *templateresolution.Context, scope *wfScope, prefix string, node *wfv1.NodeStatus, tmplHolder wfv1.TemplateReferenceHolder, includeArtifacts bool) {
	...
	_, tmpl, _, err := tmplCtx.ResolveTemplate(tmplHolder)
	if err != nil {
		woc.log.WithError(err).Debug("failed to resolve template for skipped node, outputs will not be populated in scope")
		return
	}
	if tmpl == nil {
		return
...
func (s *wfScope) resolveArtifact(art *wfv1.Artifact) (*wfv1.Artifact, error) {
	...
	if art.SubPath != "" {
		...
		resolvedSubPathAsJSON, err := template.Replace(string(subPathAsJSON), s.getParametersAny(nil), true)
		if err != nil {
			return nil, err
		}
