...
func (woc *wfOperationCtx) operate(ctx context.Context) {
	...
	err = woc.substituteParamsInVolumes(template.ToAnyMap(woc.globalParams))
	if err != nil {
...
func (woc *wfOperationCtx) assessNodeStatus(ctx context.Context, pod *apiv1.Pod, old *wfv1.NodeStatus) *wfv1.NodeStatus {
	...
	if tmpl != nil && tmpl.Outputs.HasOutputs() && new.Outputs != nil && new.Phase == wfv1.NodeSucceeded {
		...
		if outputsNotReady {
			woc.log.WithField("new.phase", new.Phase).Info("leaving phase un-changed: required outputs are not yet set")
			new.Phase = old.Phase
		}
	}
...
func (woc *wfOperationCtx) executeTemplate(ctx context.Context, nodeName string, orgTmpl wfv1.TemplateReferenceHolder, tmplCtx *templateresolution.Context, args wfv1.Arguments, opts *executeTemplateOpts) (node *wfv1.NodeStatus, err error) {
	...
	processedTmpl, err := common.ProcessArgs(resolvedTmpl, &args, woc.globalParams, localParams, false, woc.wf.Namespace, woc.controller.typedConfigMapInformer.GetIndexer())
	if err != nil {
		return woc.initializeNodeOrMarkError(node, nodeName, templateScope, orgTmpl, opts.boundaryID, opts.nodeFlag, err), err
	}
...
func (woc *wfOperationCtx) recordWorkflowPhaseChange(ctx context.Context) {
	...
	if woc.wf.Spec.WorkflowTemplateRef != nil { // not-woc-misuse
		...
		case wfv1.WorkflowSucceeded, wfv1.WorkflowFailed, wfv1.WorkflowError:
			...
			log.Infof("Recording template time")
		}
	}
}
	...
	if len(tmpl.Outputs.Parameters) > 0 {
		...
		for _, param := range tmpl.Outputs.Parameters {
			...
			if val == nil {
				...
				return nil, errors.Errorf(errors.CodeBadRequest, "output parameter %q: %q is an absent optional (skipped/omitted node output with no default)", param.Name, param.ValueFrom.Parameter)
			}
...
func processItem(tmpl template.Template, name string, index int, item wfv1.Item, obj interface{}, whenCondition string, globalScope map[string]any) (string, error) {
	...
	if err == nil && !proceed {
		...
		newStepStr, err = tmpl.Replace(lenientMap, true)
	} else {
...
func (woc *wfOperationCtx) substituteParamsInVolumes(params map[string]any) error {
	if woc.volumes == nil {
		return nil
	}
...
func (woc *wfOperationCtx) substituteGlobalVariables(params common.Parameters) error {
	...
	resolveSpec, err := template.Replace(string(wfSpec), template.ToAnyMap(params), true)
	if err != nil {
		return err
	}
