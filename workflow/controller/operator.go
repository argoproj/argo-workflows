...
func (woc *wfOperationCtx) executeTemplate(ctx context.Context, nodeName string, orgTmpl wfv1.TemplateReferenceHolder, tmplCtx *templateresolution.Context, args wfv1.Arguments, opts *executeTemplateOpts) (*wfv1.NodeStatus, error) {
	...
	case wfv1.TemplateTypeTransformation:
		node, err = woc.executeTransformation(ctx, nodeName, templateScope, processedTmpl, orgTmpl, opts)
	default:
...
func (woc *wfOperationCtx) executeTransformation(ctx context.Context, nodeName string, templateScope string, tmpl *wfv1.Template, orgTmpl wfv1.TemplateReferenceHolder, opts *executeTemplateOpts) (*wfv1.NodeStatus, error) {
	node := woc.wf.GetNodeByName(nodeName)
	if node == nil {
		node = woc.initializeExecutableNode(nodeName, wfv1.NodeTypePod, templateScope, tmpl, orgTmpl, opts.boundaryID, wfv1.NodePending)
	} else if !node.Pending() {
	...
	withArtifacts, err := json.Marshal(tmpl.Transformation.WithArtifacts)
	if err != nil {
		return node, fmt.Errorf("could not marhsal withArtifacts in transformation: %w", err)
	}
