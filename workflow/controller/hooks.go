...
func (woc *wfOperationCtx) executeWfLifeCycleHook(ctx context.Context, tmplCtx *templateresolution.Context) (bool, error) {
	...
	for hookName, hook := range woc.execWf.Spec.Hooks {
		...
		if execute || hookedNode != nil {
			...
			hookNode, err := woc.executeTemplate(ctx, hookNodeName, &wfv1.WorkflowStep{Template: hook.Template, TemplateRef: hook.TemplateRef}, tmplCtx, hook.Arguments, &executeTemplateOpts{hooked: true})
			if err != nil {
				return true, err
			}
