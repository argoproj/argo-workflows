...
A plugin that does not implement these methods keeps working unchanged: the server buffers the content to a temp file and calls the existing `Save`, so this is backward compatible.
