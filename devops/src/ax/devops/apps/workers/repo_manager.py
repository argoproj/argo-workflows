...
def main():
    ...
    parser.add_argument('-c', '--concurrency', dest='concurrency', type=int, default=DEFAULT_CONCURRENCY, help="specify concurrency")
    parser.add_argument('-i', '--interval', dest='interval', type=int, default=DEFAULT_INTERVAL, help="specify interval")
    parser.add_argument('--version', action='version', version="%(prog)s {}".format(__version__))
    args = parser.parse_args()

    logging.basicConfig(stream=sys.stdout, level=args.log_level,
                        format="%(asctime)s %(levelname)s %(threadName)s %(lineno)d %(name)s: %(message)s",
    ...
    repos_manager = RepoManager(args.concurrency, args.interval)
    repos_manager.run()
