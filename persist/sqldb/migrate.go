...
func (m migrate) Exec(ctx context.Context) error {
    ...
    primary key (id, namespace)
		...
		ansiSQLChange(`update table argo_archived_workflows set clustername = ` + m.clusterName + ` where clustername is null`),
		ternary(dbType == "mysql",
		...
		ansiSQLChange(`update table ` + m.tableName + ` set clustername = ` + m.clusterName + ` where clustername is null`),
		ternary(dbType == "mysql",
