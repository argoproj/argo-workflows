...
// NewOffloadNodeStatusRepo creates an OffloadNodeStatusRepo for the specified cluster and table.
// It reads OFFLOAD_NODE_STATUS_TTL from the environment (default 5m) to set the offload record TTL and logs the configured value.
...
func (wdc *nodeOffloadRepo) Save(ctx context.Context, uid, namespace string, nodes wfv1.Nodes) (string, error) {
	...
	err = wdc.sessionProxy.With(ctx, func(session db.Session) error {
		_, err := session.Collection(wdc.tableName).Insert(record)
		if err != nil {
