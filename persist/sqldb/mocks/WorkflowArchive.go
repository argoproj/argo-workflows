...
// ListWorkflows provides a mock function with given fields: namespace, namePrefix, minStartAt, maxStartAt, labelRequirements, limit, offset
func (_m *WorkflowArchive) ListWorkflows(namespace string, namePrefix string, minStartAt time.Time, maxStartAt time.Time, labelRequirements labels.Requirements, limit int, offset int) (v1alpha1.Workflows, error) {
	ret := _m.Called(namespace, namePrefix, minStartAt, maxStartAt, labelRequirements, limit, offset)
	...
	if rf, ok := ret.Get(0).(func(string, string, time.Time, time.Time, labels.Requirements, int, int) v1alpha1.Workflows); ok {
		r0 = rf(namespace, namePrefix, minStartAt, maxStartAt, labelRequirements, limit, offset)
	} else {
		if ret.Get(0) != nil {
	...
	if rf, ok := ret.Get(1).(func(string, string, time.Time, time.Time, labels.Requirements, int, int) error); ok {
		r1 = rf(namespace, namePrefix, minStartAt, maxStartAt, labelRequirements, limit, offset)
	} else {
		r1 = ret.Error(1)
	}
