...
func (s *ServiceProviderSettings) GetAuthnRequest() *AuthnRequest {
	r := NewAuthnRequest(s.SPSignRequest)
...
func NewAuthnRequest(signRequest bool) *AuthnRequest {
	id := "id" + strings.Replace(id(), "-", "", -1)
	...
	fmt.Println("sign_request:", signRequest)
	// Only populate the signature section when sign is required
	if signRequest {
