...
spec:
  ...
  - name: retry-backoff
    retryStrategy:
      ...
      scheduleOnDifferentHostNodesLabel: "kubernetes.io/hostname"
    container:
      image: python:alpine3.6
      command: ["python", -c]
...
* `scheduleOnDifferentHostNodesLabel` is a Kubernetes node's label "hostname" and specifies if the container will be retried on a different hostname at every retry.
