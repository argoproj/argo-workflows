...
# * `local` run the workflow–controller and argo-servers as single replicas on the local machine (default)
# * `kubernetes` run 2 replicas of the workflow-controller and argo-server on the Kubernetes cluster
RUN_MODE              := local
