...
# SRC variable removed as it relied on GOPATH
VERSION               := latest
...
# Assumed to be in PATH or provided by the environment
...
GO_BUILD_TAGS               := $(shell grep -rh '//go:build' --include='*.go' . 2>/dev/null | grep -v vendor | sed 's|.*//go:build ||' | tr '&|!() ' '\n' | grep -v '^$$' | grep -vE '^(windows|darwin|ignore_autogenerated)$$' | sort -u | tr '\n' ',' | sed 's/,$$//')
...
codegen: types swagger manifests $(GENERATED_DOCS) ## Generate code via `go generate`, as well as SDKs
	go generate ./...
	GOFLAGS=-mod=mod mockery --config .mockery.yaml
...
lint-go: ui/dist/app/index.html
	...
	golangci-lint run --fix --verbose --build-tags="$(GO_BUILD_TAGS)"
...
pkg/apis/workflow/v1alpha1/zz_generated.deepcopy.go: $(TYPES)
	CODEGEN_DIR=$$(go list -mod=mod -m -f '{{.Dir}}' k8s.io/code-generator@v0.33.1); \
...
docs-spellcheck: docs/metrics.md ## Spell check docs
	# check docs for spelling mistakes
	mdspell --ignore-numbers --ignore-acronyms --en-us --no-suggestions --report $(shell find docs -name '*.md' -not -name upgrading.md -not -name README.md -not -name fields.md -not -name workflow-controller-configmap.md -not -name upgrading.md -not -name executor_swagger.md -not -path '*/cli/*' -not -name tested-kubernetes-versions.md -not -name new-features.md)
...
.PHONY: docs-lint
docs-lint: docs/metrics.md
	# lint docs
	markdownlint docs --fix --ignore docs/fields.md --ignore docs/executor_swagger.md --ignore docs/cli --ignore docs/walk-through/the-structure-of-workflow-specs.md --ignore docs/tested-kubernetes-versions.md --ignore docs/go-sdk-guide.md

.PHONY: docs
docs: docs-spellcheck docs-lint ## Build docs
...
features-validate: hack/featuregen/featuregen
	...
	$< validate
	$< update --dry |  tail +4 | markdownlint -s
...
features-update: hack/featuregen/featuregen
	...
	markdownlint ./docs/new-features.md
