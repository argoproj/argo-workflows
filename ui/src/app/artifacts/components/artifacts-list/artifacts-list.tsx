...
export class ArtifactsList extends BasePage<RouteComponentProps<any>, State> {
    private get wfInput() {
    ...
    public componentDidMount(): void {
        this.fetchArtifacts(this.state.namespace, this.state.selectedPhases, this.state.selectedLabels);
    }
    ...
    private fetchArtifacts(namespace: string, selectedPhases: string[], selectedLabels: string[]): void {
        if (this.subscription) {
            this.subscription.unsubscribe();
        }
        let artifactList;
        ...
        if (!this.state.initialized) {
            artifactList = services.info.getInfo().then(info => {
                ...
                return services.artifacts.list(newNamespace, selectedPhases, selectedLabels);
            });
        } else {
            if (this.state.managedNamespace) {
                newNamespace = this.state.namespace;
            }
            artifactList = services.workflows.list(newNamespace, selectedPhases, selectedLabels);
        }
        artifactList
                        ...
                        {this.state.workflows.map(workflow => (
                            <div key={workflow.metadata.name}>
                                <Link to={uiUrl(`workflows/${workflow.metadata.namespace}/${workflow.metadata.name}`)}>
                                    <ArtifactListItem workflow={workflow} archived={false} />
                                </Link>
                            </div>
                        ))}
                    </div>
