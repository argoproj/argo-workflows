...
MemoizationConfig contains memoization cache configuration for database-backed storage. When configured, cache entries are stored in the configured database table instead of ConfigMaps. If `TableName` is omitted, it defaults to `cache_entries`.
...
| `ConnectionPool`    | [`ConnectionPool`](#connectionpool)       | Pooled connection settings for all types of database connections                                                     |
| `DBReconnectConfig` | [`DBReconnectConfig`](#dbreconnectconfig) | DBReconnectConfig are configuration options for database retries and reconnections                                   |
| `TableName`         | `string`                                  | TableName is the name of the table to use for memoization cache entries. Defaults to `cache_entries` when omitted. |
| `SkipMigration`     | `bool`                                    | SkipMigration skips automatic database migration on startup.                                                         |
