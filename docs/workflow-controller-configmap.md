...
MemoizationConfig contains memoization cache configuration for database-backed storage. When configured, cache entries are stored in the specified database table instead of ConfigMaps.
...
| `ConnectionPool`    | [`ConnectionPool`](#connectionpool)       | Pooled connection settings for all types of database connections                                                     |
| `DBReconnectConfig` | [`DBReconnectConfig`](#dbreconnectconfig) | DBReconnectConfig are configuration options for database retries and reconnections                                   |
| `TableName`         | `string`                                  | TableName is the name of the table to use for memoization cache entries. Defaults to "memoization_cache" if not set. |
