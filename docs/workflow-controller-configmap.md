...
| `EnablePKCEAuthentication` | `bool`                                                                                                                      | EnablePKCEAuthentication enables PKCE (RFC 7636) on the OAuth2 authorization code flow. Disabled by default for backward compatibility. Enabling PKCE is recommended by the OAuth 2.0 Security Best Current Practice (RFC 9700) for all clients, including confidential server-side clients, and is required by certain authorization servers. |
