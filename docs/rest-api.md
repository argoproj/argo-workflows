...
The server can be configured with or without client auth (`--auth-type client`). When it is disabled, then clients must pass their Kubeconfig base 64 encoded in the HTTP `Authorization` header:
