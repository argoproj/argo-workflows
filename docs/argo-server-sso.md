...
If your OIDC provider returns a different object/data structure for groups, you can ignore the `customGroupClaimName` key and still use that claim to evaluate expression for binding service account but that information will not be available in UI for display.
