...
| `workflow.failures`                       | runtime       | json           | exit-handler                                               | Failed-node descriptors. Wire format: a strconv.Quote-wrapped JSON string (operator.go:453) — consumers must JSON-decode twice. When no nodes have failed, the value is the literal 6-character string "null" (with quotes), not an empty array.                                                                                                                                        |
...
| `workflow.priority`                       | global        | string         | workflow-start, pre-dispatch, during-execute, exit-handler | Workflow priority. Conditional — resolves only when spec.priority is set; otherwise both lint (validate.go:250-252) and runtime (operator.go:662-664) treat the reference as undefined (no empty/zero fallback).                                                                                                                                                                        |
...
| `workflow.priority`                  | string | workflow-start, pre-dispatch, during-execute, exit-handler | Workflow priority. Conditional — resolves only when spec.priority is set; otherwise both lint (validate.go:250-252) and runtime (operator.go:662-664) treat the reference as undefined (no empty/zero fallback).                                                                                                                                                                        |
...
| `workflow.failures` | json   | exit-handler                               | Failed-node descriptors. Wire format: a strconv.Quote-wrapped JSON string (operator.go:453) — consumers must JSON-decode twice. When no nodes have failed, the value is the literal 6-character string "null" (with quotes), not an empty array. |
