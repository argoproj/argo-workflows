...
| `WORKFLOW_COMPRESSION_ALGORITHM`         | `string`            | `gzip`                                                                                      | The algorithm used to compress node statuses of large workflows: "`gzip`", "`zstd`" (~18% smaller), or "`brotli`" (~33% smaller, slower to compress). Decompression auto-detects the algorithm, but workflows written with a non-`gzip` algorithm cannot be read by versions without support for it, so only change this after all components are upgraded. |
| `WORKFLOW_COMPRESSION_BROTLI_QUALITY`    | `int`               | `9`                                                                                         | The brotli quality (0-11) used when `WORKFLOW_COMPRESSION_ALGORITHM` is `brotli`. Higher is smaller but slower to compress; 10+ is drastically slower for a modest size gain.                                                                                                            |
