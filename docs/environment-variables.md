...
| `TASK_RESULT_TIMEOUT_DURATION`           | `time.Duration`     | `10m`                                                                                       | The duration of time before a node is marked completed but the task result has not arrived yet. Prefer to use this over `RECENTLY_DELETED_POD_DURATION` since this accounts for pods that are not removed but haven't produced `taskresults`                             |
