...
| `WORKFLOW_COMPRESSION_ALGORITHM`         | `string`            | `gzip`                                                                                      | The algorithm used to compress node statuses of large workflows: "`gzip`" or "`zstd`" (~18% smaller). Decompression auto-detects the algorithm, but workflows written with `zstd` cannot be read by versions without `zstd` support, so only enable it after all components are upgraded. |
