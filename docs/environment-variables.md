...
| `SEMAPHORE_WAITING_HOLDERS_DISPLAY_LIMIT` | `int`              | `10`                                                                                        | Maximum number of holders to display in semaphore waiting status. Helps reduce memory usage when many workflows wait on the same semaphore. Set to 0 or negative to disable the limit (show all holders).                                                                 |
