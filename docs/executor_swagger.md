...
| multipartConcurrency | int32 (formatted integer)| `int32` |  | | MultipartConcurrency is the number of concurrent multipart uploads.</br>If not set, defaults to 4. |  |
| multipartPartSize | int64 (formatted integer)| `int64` |  | | MultipartPartSize is the size of each part in a multipart upload (in bytes).</br>If not set, defaults to 5MB (5242880 bytes).</br>Minimum: 5MB (5242880 bytes), Maximum: 5GB (5368709120 bytes). |  |
| parallelTransfers | int32 (formatted integer)| `int32` |  | | ParallelTransfers is the number of parallel transfers to use for S3 operations.</br>If not set, defaults to 1. |  |
