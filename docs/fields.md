...
|`multipartConcurrency`|`integer`|MultipartConcurrency is the number of concurrent multipart uploads. If not set, defaults to 4.|
|`multipartPartSize`|`integer`|MultipartPartSize is the size of each part in a multipart upload (in bytes). If not set, defaults to 5MB (5242880 bytes). Minimum: 5MB (5242880 bytes), Maximum: 5GB (5368709120 bytes).|
|`parallelTransfers`|`integer`|ParallelTransfers is the number of parallel transfers to use for S3 operations. If not set, defaults to 1.|
