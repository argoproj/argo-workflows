...
|`multipartPartSize`|`integer`|MultipartPartSize is the size of each part in a multipart upload (in bytes). If not set, defaults to 5MB (5242880 bytes). Minimum: 5MB (5242880 bytes), Maximum: 5GB (5368709120 bytes). Can be overridden by ARGO_S3_MULTIPART_PART_SIZE environment variable.|
|`parallelTransfers`|`integer`|ParallelTransfers is the number of parallel transfers to use for S3 operations. If not set, defaults to 1. Can be overridden by ARGO_S3_PARALLEL_TRANSFERS environment variable.|
