...
|`priority`|`integer`|Priority controls which step is scheduled first. Higher values will be processed first.|
...
|~`onExit`~|~`string`~|~OnExit is a template reference which is invoked at the end of the template, irrespective of the success, failure, or error of the primary template.~ DEPRECATED: Use Hooks[exit].Template instead.|
|`priority`|`integer`|Priority controls which task is scheduled first. Higher values will be processed first.|
